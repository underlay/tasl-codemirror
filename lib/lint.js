...
const errorUnit_js_1 = require("./errorUnit.js");
        ...
        if (cursor.type.isError) {
            ...
            if (node !== null) {
                ...
                parseState.schema[uri] = {
                    type: "coproduct",
                    options: {
                        [apg_1.ns.none]: { type: "unit" },
                        [apg_1.ns.some]: {
                            type: "product",
                            components: {
                                [apg_1.ns.head]: head,
                                [apg_1.ns.tail]: { type: "reference", value: uri },
                            },
                        },
                    },
                };
...
function getType(state, diagnostics, node) {
    if (node.name === "Variable") {
        ...
        if (value in state.types) {
            ...
            diagnostics.push({ from, to, message, severity: "error" });
            return errorUnit_js_1.errorUnit;
        }
    }
    else if (node.name === "Optional") {
        const expression = node.getChild("Expression");
        const type = expression === null ? errorUnit_js_1.errorUnit : getType(state, diagnostics, expression);
        return {
            type: "coproduct",
