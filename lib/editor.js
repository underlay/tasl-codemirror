...
function Editor({ initialValue, onChange, readOnly, noLint, }) {
    ...
    react_1.useEffect(() => {
        if (div.current) {
            ...
            if (!noLint) {
                extensions.push(lint_js_1.makeSchemaLinter(handleChange));
            }
            if (readOnly) {
                extensions.push(basic_setup_1.EditorView.editable.of(false));
            }
            state.current = basic_setup_1.EditorState.create({ doc: initialValue, extensions });
            view.current = new basic_setup_1.EditorView({
                state: state.current,
                parent: div.current,
            });
            if (!noLint) {
